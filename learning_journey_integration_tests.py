...
from learning_journey import app, db, Learning_Journey, Learning_Journey_Courses, Courses, Staff, Registration, Course_skills
...
class TestRemoveLearningJourney(TestApp):
    def test_remove_learning_journey(self):
        lj1 = Learning_Journey(lj_id=1,title="Design Engineer Journey",role_id=3,staff_id=150166)
        db.session.add(lj1)
        db.session.commit()
        
        response = self.client.delete("/removelearningjourney/1",
                                    content_type='application/json')
